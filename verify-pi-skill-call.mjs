...
const LOG = "D:\\code_kj\\Agent工具开发\\AgentDesk\\.devlogs\\m07-t02.log"
...
const res = await fetch(`${BASE}/api/send`, { method: "POST", headers: { "content-type": "application/json; charset=utf-8" }, body: JSON.stringify({ message: "运行 pi-echo skill，按 skill 指示回复", directory: WS }), signal: AbortSignal.timeout(30000) })
const sid = (await res.json()).sessionId
log("session: " + sid)
const native = sid.replace(/^pi:/, "")

// listen SSE for reply
